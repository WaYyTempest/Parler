; Custom NSIS template for Parler with portable mode support.
...
Function PageLeaveInstallType
  ...
  ${If} $0 = ${BST_CHECKED}
    ...
    ; --- PORTABLE MODE --- Switch default directory to Desktop\Parler for portable
...
Function .onInit
  ...
  ; or a legacy marker created by older releases. Require Data/
  ...
  ${AndIf} ${FileExists} "$INSTDIR\portable"
    ...
    FileRead $1 $2
    FileClose $1
    ${If} $2 == "Parler Portable Mode"
      StrCpy $PortableMode 1
    ; Keep accepting the upstream marker so existing portable installs migrate.
...
Section Install
  ...
  ${If} $PortableMode = 1
    ...
    FileWrite $0 "Parler Portable Mode"
    FileClose $0
