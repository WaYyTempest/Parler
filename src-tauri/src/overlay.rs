...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    ...
    // KDE Wayland has shown protocol instability with layer-shell. Preserve
    // Parler's regular always-on-top fallback for this desktop combination.
...
fn get_fallback_monitor(
    ...
    if let Some(monitor) = monitors.iter().max_by_key(|monitor| {
...
fn get_monitor_with_cursor(app_handle: &AppHandle) -> Option<tauri::Monitor> {
    ...
    if let Some((mouse_x, mouse_y)) = input::get_cursor_position(app_handle) {
        ...
        for monitor in &monitors {
            ...
            if is_point_within_monitor((mouse_x, mouse_y), &logical_position, &logical_size) {
                return Some(monitor.clone());
            }
