...
struct ModelRestoreGuard {
    manager: Arc<TranscriptionManager>,
    model_id: Option<String>,
}

impl Drop for ModelRestoreGuard {
    fn drop(&mut self) {
        if let Some(model_id) = self.model_id.take() {
            debug!("Restoring original model: {}", model_id);
            if let Err(error) = self.manager.load_model(&model_id) {
                warn!("Failed to restore original model '{}': {}", model_id, error);
            }
        }
    }
}
...
pub(crate) async fn process_transcription_output(
    ...
    process_transcription_output_with_action(app, transcription, post_process, None).await
}

pub(crate) async fn process_transcription_output_with_action(
    app: &AppHandle,
    transcription: &str,
    post_process: bool,
    ...
    let mut post_process_action_key: Option<u8> = None;
    ...
    let processed = if let Some(action) = selected_action.as_ref() {
        process_action(
            &settings,
            &final_text,
            &action.prompt,
    ...
    if let Some(processed_text) = processed {
        ...
        if let Some(action) = selected_action {
            ...
            post_process_action_key = selected_action_key;
        } else if post_process {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id, cancel_generation) {
                ...
                if samples.is_empty() {
                    ...
                    let file_name = format!("parler-{}.wav", chrono::Utc::now().timestamp());
                    ...
                    // Transcribe concurrently with WAV save. Long recordings can
                    // opt into Parler's dedicated long-audio model; otherwise a
                    // live stream is finalized and used when it produced text.
                    ...
                    let long_audio_model = settings_for_model.long_audio_model.filter(|model_id| {
                        duration_seconds > settings_for_model.long_audio_threshold_seconds
                            && original_model.as_deref() != Some(model_id.as_str())
                    });
                    let mut restore_model_id = None;

                    let transcription_result = if let Some(long_model_id) = long_audio_model {
                        debug!(
                        ...
                        // Finalizing first returns any leased streaming engine before
                        // replacing the loaded model. Its text is intentionally ignored
                        // because the configured long-audio model must handle this clip.
                        match tm.finalize_stream() {
                            Err(error) => Err(error),
                            Ok(_) => match tm.load_model(&long_model_id) {
                                Ok(()) => {
                                    restore_model_id = original_model;
                    ...
                    let _model_restore_guard = ModelRestoreGuard {
                        manager: Arc::clone(&tm),
                    ...
                    match transcription_result {
                        Ok(transcription) => {
                            ...
                            if post_process || selected_action_key.is_some() {
                                if use_streaming_overlay {
                            ...
                            let Some(processed) = complete_unless_cancelled(
                                process_transcription_output_with_action(
                                    &ah,
                                    &transcription,
                                    post_process,
                                    selected_action_key,
                            ...
                            if wav_saved {
                                if let Err(err) = hm.save_entry(
                                    ...
                                    processed.post_process_action_key,
                                ) {
                        ...
                        Err(err) => {
                            ...
                            // message is also in parler.log via the line above.
