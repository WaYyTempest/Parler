fn main() {
    ...
    // $ORIGIN-relative rpath into the `parler` binary so it finds libtranscribe
    // next to it in the package — deb/rpm install into the app-private
    // `/usr/lib/Parler` (the dir tauri already uses for resources; keeps
    // Parler's libs out of the ldconfig-scanned `/usr/lib`, issue #1639) while
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("linux") {
        println!("cargo:rustc-link-arg=-Wl,-rpath,$ORIGIN/../lib/Parler:$ORIGIN/../lib");
    }
    ...
    // next to Parler.exe so the app loads our baseline build instead of statically
...
///
/// Parler's native stack links the VC++ runtime dynamically (/MD). Shipping the
/// DLLs beside `parler.exe` covers machines with no redistributable installed and
...
/// DLL families Parler imports and no-ops when the env var is unset.
        ...
        if !copied.iter().any(|n| n == required) {
            panic!(
                "HANDY_VC_REDIST_DIRS is set but {required} was not found in it; \
                 the app-local VC++ runtime would be incomplete and Parler would \
...
/// `Parler.exe` (Windows resolves DLLs from the executable's directory).
///
...
///
/// Where the staged dir lands: Windows bundles it beside `parler.exe` (DLLs resolve
/// from the exe dir); Linux deb/rpm map it into the app-private `/usr/lib/Parler`
